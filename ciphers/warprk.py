...
class WarpRKCipher(AbstractCipher):
    """
    Represents the related-key differential behaviour of Warp and can be used
    to find related-key differential characteristics for the given parameters.
    """
    ...
    def createSTP(self, stp_filename, parameters):
        ...
        with open(stp_filename, 'w') as stp_file:
            ...
            # Setup variables
            # x = input, s = S-Box layer output, p = permutation layer input, k = key difference
            ...
            for i in range(rounds):
                self.setupWarpRound(stp_file, x[i], s[i], p[i], k[i%2], x[i+1], 
                                     w[i], wordsize)

            # No all zero characteristic
            stpcommands.assertNonZero(stp_file, x, wordsize)
            stpcommands.assertNonZero(stp_file, k, wordsize)
    ...
    def setupWarpRound(self, stp_file, x_in, s, p, k, x_out, w, wordsize):
        """
        Model for (related-key) differential behaviour of one round WARP
        """
        command = ""
        ...
        command += "ASSERT({0}[7:4]     = BVXOR({1}[7:4],   BVXOR({2}[3:0],     {3}[3:0]    )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[15:12]   = BVXOR({1}[15:12], BVXOR({2}[7:4],     {3}[7:4]    )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[23:20]   = BVXOR({1}[23:20], BVXOR({2}[11:8],    {3}[11:8]   )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[31:28]   = BVXOR({1}[31:28], BVXOR({2}[15:12],   {3}[15:12]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[39:36]   = BVXOR({1}[39:36], BVXOR({2}[19:16],   {3}[19:16]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[47:44]   = BVXOR({1}[47:44], BVXOR({2}[23:20],   {3}[23:20]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[55:52]   = BVXOR({1}[55:52], BVXOR({2}[27:24],   {3}[27:24]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[63:60]   = BVXOR({1}[63:60], BVXOR({2}[31:28],   {3}[31:28]  )));\n".format(p, x_in, s, k)
        
        command += "ASSERT({0}[71:68]   = BVXOR({1}[71:68], BVXOR({2}[35:32],   {3}[35:32]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[79:76]   = BVXOR({1}[79:76], BVXOR({2}[39:36],   {3}[39:36]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[87:84]   = BVXOR({1}[87:84], BVXOR({2}[43:40],   {3}[43:40]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[95:92]   = BVXOR({1}[95:92], BVXOR({2}[47:44],   {3}[47:44]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[103:100] = BVXOR({1}[103:100], BVXOR({2}[51:48], {3}[51:48]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[111:108] = BVXOR({1}[111:108], BVXOR({2}[55:52], {3}[55:52]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[119:116] = BVXOR({1}[119:116], BVXOR({2}[59:56], {3}[59:56]  )));\n".format(p, x_in, s, k)
        command += "ASSERT({0}[127:124] = BVXOR({1}[127:124], BVXOR({2}[63:60], {3}[63:60]  )));\n".format(p, x_in, s, k)
