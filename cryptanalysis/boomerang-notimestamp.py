...
def boomerangTrail(cipher, parameters, boomerangFace="upper", switchInput=""):
    """
    ...
    while not search.reachedTimelimit(start_time, parameters["timelimit"]) and \
        ...
        stp_file = "tmp/{}-{}{}-{}.stp".format(boomerangFace, cipher.name,
                                         parameters["wordsize"], parameters[trail])
...
def boomerangDifferential(cipher, parameters, input, output, weight, boomerangFace="upper"):
    """
    ...
    while not search.reachedTimelimit(start_time, parameters["timelimit"]) and \
        ...
        stp_file = "tmp/{}{}.stp".format(cipher.name, trail)
        cipher.createSTP(stp_file, parameters)
...
def feistelBoomerangTrailSearch(cipher, parameters, boomerangProb = 0):
    ...
    #Search Upper Trail
    upperCharacteristic = boomerangTrail(cipher, parameters, "upper")
    ...
    while not search.reachedTimelimit(start_time, parameters["timelimit"]) and \
        ...
        if switchProb != 0:
            ...
            if diff_upper == 0:
                diff_upper = boomerangDifferential(cipher, parameters, alpha, beta, upperWeight, "upper")
            while diff_upper == 0: #TODO: Change later
                print("Upper clustering failed.")
                diff_upper = boomerangDifferential(cipher, parameters, alpha, beta, upperWeight, "upper")
                #continue
            diff_lower = boomerangDifferential(cipher, parameters, gamma, delta, lowerWeight, "lower")
            while diff_lower == 0: #TODO: Change later
                print("Lower clustering failed.")
                diff_lower = boomerangDifferential(cipher, parameters, gamma, delta, lowerWeight, "lower")
                #continue
